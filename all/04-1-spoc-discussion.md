...
1. 寄存器、高速缓存、内存、外存的访问特征？
   ...
   寄存器：速度最快，延迟最低，容量小，有专用和通用

   cache：速度快，有L1，L2，L3等等，容量也较小，且L1-3逐渐增大

   内存：速度较快，带宽较大，容量大，可以支持到 TB 级
...
3. 在你写程序时遇到过内存不够的情况吗？尝试过什么解决方法？
   ...
   有，优化程序，使用swap机制；使用动态链接库

### 8.2 覆盖和交换

1. 什么是覆盖技术？使用覆盖技术的程序开发者的主要工作是什么？

   将程序模块化，找出不同时使用的模块进行分组，每组内分配最大块的内存，所有程序块在使用时动态加载入内存块中，节省单个程序的总内存使用

   对程序进行分块，并指定好调用关系。

2. 什么是交换技术？覆盖与交换有什么不同？

   将不使用的进程内存从内存中国呢交换出去，省出空间给其他运行的程序使用。
...
1. 什么是时间局部性、空间局部性和分支局部性？
   ...
   时间局部性：程序访问的指令或者数据一段时间内可能会被重复访问

   空间局部性：程序访问内存中某个空间的地址，也可能访问该地址周围的地址空间。

   分支局部性：程序如果跳转，下一次也很可能沿之前的跳转路径继续跳转执行

2. 如何提高程序执行时的局部性特征？

   使用 cache，一次性加载一块，预加载技术
...
1. 什么是虚拟存储？它与覆盖和交换的区别是什么？它有什么好处和挑战？
   ...
   将逻辑空间与物理空间隔离，建立逻辑地址与物理地址的对应关系，在加载时可以只映射部分地址空间。

   以页为基本单位进行交换，实际内存空间不连续，可以通过交换技术使虚拟空间大于实际地址空间

2. 虚拟存储需要什么样的支持技术？

   需要地址转换机制，并且硬件可以实现权限的安全性检查

   操作系统需要实现页表机制，并在加载时设置好页表的初始值，设置好缺页异常服务例程
...
### 8.5 虚拟页式存储
 ...
 1. 什么是虚拟页式存储？缺页中断处理的功能是什么？
    ...
    进行虚拟页地址与实际物理页帧地址的一一对应

    当程序访问一个合法的页但是该页已经被交换出内存在swap空间时，使用替换算法将该页从swap空间换回内存，并建立好相应映射关系，完成程序合法的内存访问。如果页不存在则报错。

 1. 为了支持虚拟页式存储的实现，页表项有什么修改？

    需要有访问位，dirty位，present 位，需要能够表示交换在swap空间的哪一个区域，在交换回内存时能找到相应的页；有一位表示该页是否允许置换到外存swap分区中

 1. 页式存储和虚拟页式存储的区别是什么？

    虚拟页式存储完成了虚拟页与物理页的转换，程序的访问都是基于虚拟地址的访问。
...
一般不会出现这种情况，因为缺页服务例程中不能够再次缺页，否则无法结束这个过程。只有可能在其他的异常处理过程中出现缺页而出发缺页服务例程，为 double fault，可以按照压栈顺序处理。否则内核错误，进程退出
