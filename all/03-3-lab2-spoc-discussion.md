...
2. 不同特权级有什么区别？
   ...
   高特权级（数值小的）权限高，可以访问低特权级的地址，反之不成立
...
3. 请说明CPL、DPL和RPL在中断响应、函数调用和指令执行时的作用。
   ...
   CPL：表示当前 CPU 执行程序代码特权级

   DPL：表示访问该段数据所需要的最低特权级

   RPL：用于限制CPL，表示程序希望以怎样的特权级访问数据。用于限制高特权级应用访问数据时的权限
...
2. 在什么情况下会出现特权级切换？
   ...
   中断异常处理和恢复现场时，内核加载初始化特权级时，内核线程创建用户进程并开始执行用户代码时

3. int指令在ring0和ring3的执行行为有什么不同？

   在 ring3 表示系统调用，需要保护用户现场，压栈要压入 esp 和 ss，在 ring0 则不需要


4. 如何利用int和iret指令完成不同特权级的切换？

   int 在最后会将特权级从用户态转变为内核态，而在恢复用户现场时需要先将栈中相应的CS段，DS段，esp，wip等设置好，权限设置为用户态权限，最后通过 iret 根据栈中内容恢复现场并切换回用户态权限


5. TSS和Task Register的作用是什么？

   TSS 用于保存内核栈信息等异常处理和权限转换的信息，保存了 ring0-2 的栈信息，用于实现栈的切换

   Task Register 保存了 TSS 段的基地址
